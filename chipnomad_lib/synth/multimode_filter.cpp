...
void MultimodeFilter::init(float sampleRate) {
  ...
  reset();
  configure(false, 0, false, kMaxCutoff, 0.0f);
}

void MultimodeFilter::configure(bool enabled, uint8_t mode, bool slope24dB,
...
float MultimodeFilter::processStage(float input, int channel, int stage) {
  ...
  return mode_ == 1 ? band : (mode_ == 2 ? high : low);
}
...
float MultimodeFilter::process(float input, int channel) {
  ...
  return slope24dB_ ? processStage(output, channel, 1) : output;
}

...
namespace rosic
{
  ...
  */
  ...
  public:
    ...
    INLINE double getSampleWithExternalInput(double input);
    ...
    void trigger(int noteNumber, bool accent) { triggerNote(noteNumber, accent); }
    void slide(int noteNumber, bool accent) { slideToNote(noteNumber, accent); }
    void release() { releaseNote(currentNote); }
  ...
  INLINE double Open303::getSampleWithExternalInput(double input)
  {
    // Keep the original 303 VCF/VCA/envelope path; only replace the VCO source.
    if( idle ) return 0.0;
    double instFreq = pitchSlewLimiter.getSample(oscFreq);
    oscillator.setFrequency(instFreq*pitchWheelFactor);
    oscillator.calculateIncrement();
    double mainEnvOut = mainEnv.getSample();
    double tmp1 = n1 * rc1.getSample(mainEnvOut), tmp2 = accentGain > 0.0 ? mainEnvOut : 0.0;
    tmp2 = n2 * rc2.getSample(tmp2);
    tmp1 = envScaler * (tmp1 - envOffset); tmp2 = accentGain * tmp2;
    filter.setCutoff(cutoff * pow(2.0, tmp1 + tmp2));
    double ampEnvOut = ampEnv.getSample();
    if( ampEnv.isNoteOn() ) ampEnvOut += 0.45*mainEnvOut + accentGain*4.0*mainEnvOut;
    ampEnvOut = ampDeClicker.getSample(ampEnvOut);
    double tmp = 0.0;
    for(int i=1; i<=oversampling; i++) {
      tmp = highpass1.getSample(-input);
      tmp = filter.getSample(tmp);
      tmp = antiAliasFilter.getSample(tmp);
    }
    tmp = allpass.getSample(tmp); tmp = highpass2.getSample(tmp); tmp = notch.getSample(tmp);
    idle = false;
    return tmp * ampEnvOut * ampScaler;
  }

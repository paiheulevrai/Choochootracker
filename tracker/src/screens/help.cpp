...
const char* helpFXHint(uint8_t* fx, int isTable, uint8_t instrumentIdx) {
  ...
  switch ((enum FX)fx[0]) {
    ...
    case fxBIA: snprintf(buffer, bufferSize, "BYOWTBL index A %hhu", fx[1]); break;
    case fxBIB: snprintf(buffer, bufferSize, "BYOWTBL index B %hhu", fx[1]); break;
    ...
    case fxEDC: snprintf(buffer, bufferSize, "Envelope decay %hhu", fx[1]); break;
    case fxESU: snprintf(buffer, bufferSize, "Envelope sustain %hhu", fx[1]); break;
    case fxERL: snprintf(buffer, bufferSize, "Envelope release %hhu", fx[1]); break;
    case fxESH: snprintf(buffer, bufferSize, "Envelope shape %hhu", fx[1]); break;
    case fxTDC: snprintf(buffer, bufferSize, "Trigger decay %hhu", fx[1]); break;
    case fxTCL: snprintf(buffer, bufferSize, "Trigger color %hhu", fx[1]); break;
...
static void initFxHelpText() {
  ...
  fxHelpText[fxSRS2] = "Stereo Osc Resonance FX\nOverrides resonance until\nthe next note trigger";
  fxHelpText[fxBIA] = "BYOWTBL Index A FX\nSelects waveform A\nuntil the next note trigger";
  fxHelpText[fxBIB] = "BYOWTBL Index B FX\nSelects waveform B\nuntil the next note trigger";
  ...
  fxHelpText[fxEDC] = "ADSR Decay FX\nOverrides decay until\nthe next note trigger";
  fxHelpText[fxESU] = "ADSR Sustain FX\nOverrides sustain until\nthe next note trigger";
  fxHelpText[fxERL] = "ADSR Release FX\nOverrides release until\nthe next note trigger";
  fxHelpText[fxESH] = "ADSR Shape FX\nOverrides shape until\nthe next note trigger";
  fxHelpText[fxTDC] = "Trigger Decay FX\nPlaits trigger mode only\nuntil the next note trigger";
  fxHelpText[fxTCL] = "Trigger Color FX\nPlaits trigger mode only\nuntil the next note trigger";

...
// Callbacks for file operations
static void onLoadFileSelected(const char* path) {
  int loaded = ayWavetableLoad(path, chipnomadState->project.ayWavetables, wavetableIdx);
  if (loaded > 0) {
    ...
    if (lastSeparator) {
      ...
      if (pathLen < PATH_LENGTH) {
        strncpy(appSettings.ayWavetablePath, path, pathLen);
        appSettings.ayWavetablePath[pathLen] = 0;
      }
    }
  } else {
    screenMessage(MESSAGE_TIME, "Failed to load wavetables");
  }
  screenSetup(&screenAYWavetable, wavetableIdx);
}
...
static void onSaveFolderSelected(const char* folderPath) {
  ...
  enterNameSetup("SAVE AY WAVES", "File name:", "", onSaveNameEntered, onSaveCancelled);
...
static void onSaveNameEntered(const char* filename) {
  ...
  int success = ayWavetableSave(fullPath, chipnomadState->project.ayWavetables, wavetableIdx, wavetableSaveCount);
  if (success) {
    ...
    strncpy(appSettings.ayWavetablePath, selectedSaveFolder, PATH_LENGTH);
    appSettings.ayWavetablePath[PATH_LENGTH - 1] = 0;
  } else {
    ...
    if (col == 0 || col == 1) {
      ...
      if (action == CellEditAction::tap) {
        if (col == 0) {
          ...
          fileBrowserSetup("LOAD AY WAVES", ".aywave", appSettings.ayWavetablePath, onLoadFileSelected, onLoadCancelled);
          screenSetup(&screenFileBrowser, 0);
          return 1;
        } else if (col == 1) {
          // Save button - open folder selector first, then filename entry
          fileBrowserSetupFolderMode("SAVE AY WAVES", appSettings.ayWavetablePath, "", ".aywave", onSaveFolderSelected, onSaveCancelled);
          screenSetup(&screenFileBrowser, 0);
          return 1;
        }
      }
...
const AppScreen screenAYWavetable = {
  .init = init,
  .setup = setup,

...
int ayWavetableSave(const char* path, uint8_t wavetables[256][32], int startIndex, int count) {
...
int ayWavetableLoad(const char* path, uint8_t wavetables[256][32], int startIndex) {
  if (!path || !wavetables) return 0;
  if (startIndex < 0 || startIndex > 255) return 0;
